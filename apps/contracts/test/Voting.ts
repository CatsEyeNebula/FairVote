...
describe("SemaphoreVoting", () => {
    ...
    const wasmFilePath = `${snarkArtifactsPath}/semaphore.wasm`
    const zkeyFilePath = `${snarkArtifactsPath}/semaphore.zkey`

    before(async () => {
        const { address: verifierAddress } = await run("deploy:verifier", { logs: true, merkleTreeDepth: 20 })

        // const { address: verifierAddress } = await run("deploy:verifier", { logs: true, depth: treeDepth })
    ...
    describe("# castVote", () => {
        ...
        group.addMembers([commitment, BigInt(1)])
